...
| `catalog.js` | Public domain UI: item renderer, typed input, shape builder. No rules. |
...
A domain needs a `pool()` of items (primitives, so state stays JSON-safe), a `render(item)` for the tile, a `parse(raw)` for typed input or `input: 'builder'` for a picker, and its rules. Add it to `WEEK` in `schedule.js` to schedule it.
