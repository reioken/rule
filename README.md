...
Two daily deduction puzzles, the same for everyone, at [rule.dennis-bierreth.workers.dev](https://rule.dennis-bierreth.workers.dev).

**In or out.** A few things are already sorted through the gate by a secret rule. Send your own through, then prove you know the rule by sorting six more. Numbers, words, shapes, emoji and cards take turns through the week.

**Black box.** Three things went in and came out changed. Feed it your own, then predict four outputs. Numbers and words alternate.

- Every test or run costs one. Finish within par for three stars.
...
| `schedule.js` | Launch date, weekday domain map, difficulty map, black box map |
...
| `box.js` | Black box function libraries and generators. Server-only. |
...
| `game.js` | Both game loops, the gate, the machine log, pickers, stars, share text, stats |

## Adding a rule, a function or a domain

A black box function is an entry in `NUM_FUNCS` or `WORD_FUNCS` in `box.js`: a name, a one-line detail, `f`, and a par. Prove inputs are chosen to rule out every other function in the library.
