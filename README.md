...
A few things are already sorted through the gate by a secret rule. Send your own through, then prove you know the rule by sorting six more. Every day has an Easy, a Medium and a Hard puzzle, each with a different kind of thing: numbers, words, shapes, emoji or cards.
...
- **Same board every time, three puzzles a day.** Six examples, three in and three out. Each day draws three different kinds of thing from the day number, one per difficulty, so everyone gets the same three and there is no weekly pattern. Easy is one simple rule. Medium joins two rules with and, or, or unless, and says which. Hard joins two rules and hides the join. Finish all three for a day share with one line per puzzle.
...
- **Archive.** Any earlier day can be played from the stats dialog or with `?day=N` (add `&level=2` or `&level=3` for the Medium or Hard one). Archive plays count toward totals but not the streak, which counts days in a row with at least one solve.
...
| `schedule.js` | Launch date and the seeded daily draw: three kinds of thing per day, one per difficulty |
...
A domain needs a `pool()` of items (primitives, so state stays JSON-safe), a `render(item)` for the tile, a `parse(raw)` for typed input or a picker (`input: 'builder'`, `'pick'` or `'pair'`), and its rules. Add it to `DAILY_DOMAINS` in `schedule.js` to put it in the daily draw (a domain with too few compound rules goes in `EASY_ONLY` there). Compound rules are built automatically from the atoms.
